...
void insertion_sort(int arr[], int indeks, int n_jumlah){
    ...
    void array_sort(int arr[], int indeks){
        ...
        // untuk mengkomparasi nilai dari 2 nilai array di lokasi [indeks] dan [indeks-1],
        // jika array[indeks] lebih kecil dari array[indeks-1] maka valuenya akan ditukar
...
// print array dari kanan ke kiri
void user_output_array(int arr[], int n_jumlah){
    if (n_jumlah>0){
        --n_jumlah;
        printf ("%d ",arr[n_jumlah]);
        user_output_array(arr, n_jumlah);
    }
}
